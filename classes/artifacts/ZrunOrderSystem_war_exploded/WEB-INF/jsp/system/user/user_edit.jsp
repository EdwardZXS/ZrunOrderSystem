		...
		<table>
			...
			<tr class="info">
				...
				<c:forEach items="${roleList}" var="role">
					...
					<c:if test="${userBZ != 1 and role.PARENT_ID eq '4'}">
						<option value="${role.ROLE_ID }" <c:if test="${role.ROLE_ID == pd.ROLE_ID }">selected</c:if>>${role.ROLE_NAME }</option>
					</c:if>
			...
			<tr class="info">
				<td>
				<select class="chzn-select" name="BZ" id="BZ" data-placeholder="请选择商户" style="vertical-align:top;">
				<option value=""></option>
				<c:forEach items="${merchantlist}" var="merchant">
					<option value="${merchant.MERCHANT_ID }" <c:if test="${merchant.MERCHANT_ID == pd.BZ}">selected</c:if>>${merchant.MERCHANT_NAME}</option>
				</c:forEach>
				</select>
				</td>
			</tr>
			...
			<tr>
				<td><input type="text" name="USERNAME" id="loginname" value="${pd.USERNAME }" maxlength="32" placeholder="这里输入登录名" title="登录名"/></td>
			</tr>
			<tr>
